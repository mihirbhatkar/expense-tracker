...
const recentExpenses = asyncHandler(async (req, res) => {
	...
	res.status(200).json({
		recentExpenses,
		message: "Recent expenses returned",
	});
});
...
const searchExpenses = asyncHandler(async (req, res) => {
	const startingDate = new Date(req.body.time.start); // typeof date
	const endingDate = new Date(req.body.time.end); // typeof date
...
const oldestExpenses = asyncHandler(async (req, res) => {
	...
	for (const walletId of walletIds) {
		const oldestExpense = await Expenses.findOne({
			walletId: walletId,
		}).sort({
			dateOfExpense: 1,
		});
